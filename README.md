...
- 会话记忆与近期群图文上下文持久化到 SQLite；容器更新后仍保留，并自动压缩较早对话、限制条数和保留期。
...
Agent 2.1 起默认启用 `autoUpdate`。控制端发布新版本后，Agent 会收到带 SHA-256 的更新清单，由 `start-agent.ps1` 调用 `update-agent.ps1` 完成下载、校验、备份、覆盖和失败回滚，然后自动重启。首次从旧版升级到 2.1 需要覆盖一次新的 Agent 更新包并重新运行 `install-agent.ps1`；此后无需手工传包。
