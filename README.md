...
- AI 闲聊、图片理解、技术答疑、生图、独立会话记忆和图文上下文自动参与；明确问题无需艾特即可直接回答，并可继续追问近期图片。
- 分层人格引擎把身份、语气、回答规则、机器人覆盖、群策略、长期记忆和当前任务分开编译；后台可真实调用模型预览并保存人格快照。
- 会话记忆、用户长期记忆与近期群图文上下文持久化到 SQLite；容器更新后仍保留，会话历史自动压缩并限制保留期。
- 消息诊断中心记录每条消息的接收、排队、忽略、审核、回复、网关、Token、耗时和失败原因。
- 自定义静态回复，支持全局/指定机器人/指定群范围、三种匹配方式和消息变量，不消耗 AI 额度。
- 每群可独立设置安静、平衡、活跃模式以及人格、回复偏好、主动参与和冷场节奏；连续两次无人回应后沉寂，真人发言后自动恢复。
- 多个 OpenAI 兼容网关，自定义地址、Key、模型、优先级和超时；自动能力识别，群聊短回复 6 秒快速超时并切换备用网关，技术长回答保留独立超时。
- 网关每 15 分钟主动健康巡检，并统计最近 24 小时成功率与平均延迟。
...
- Agent 支持带 SHA-256 校验、备份和失败回滚的自动更新；QQ 群名称、群号和人数每 10 分钟自动同步，授权时可直接选择。
...
7. 在“机器人设置”填写你的 QQ 到全局管理员白名单，并在人格与节奏中配置名称、表达参数、冷场间隔、尝试次数、活跃时间和时区；“群行为”可覆盖单个群的策略。
8. 在“登录安全”修改初始密码。
...
Agent 2.1 起默认启用 `autoUpdate`。控制端发布新版本后，Agent 会收到带 SHA-256 的更新清单，由 `start-agent.ps1` 调用 `update-agent.ps1` 完成下载、校验、备份、覆盖和失败回滚，然后自动重启。2.2 起控制台会显示当前版本、目标版本和更新状态。首次从旧版升级到 2.1 需要覆盖一次新的 Agent 更新包并重新运行 `install-agent.ps1`；此后无需手工传包。

Agent 在线后，后台“节点与机器人”可手动点击群同步按钮；控制端也会每 10 分钟自动读取 NapCat 群列表。同步完成后，在“群授权”新增授权时可直接按群名称选择群号。
...
上下文自动参与默认在收到 1 条普通群消息并停顿 3 秒后判断是否接话，自动参与冷却为 10 秒；明确问题在平衡/活跃模式下会直接回答，不额外调用模型做决策。这些参数可在“机器人设置 -> 人格与节奏”统一调整，也可在“群行为”覆盖单个群。授权、激活等关键管理动作仍只响应群主、群管理员和全局管理员。
