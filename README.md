...
自定义静态回复在“机器人设置 -> 自定义回复”维护，可选择全部或指定机器人、全部授权群或指定群，并支持完全一致、消息开头和消息包含三种匹配方式。回复正文可使用 `{user}`、`{qq}`、`{group}`、`{bot}` 变量。
...
后台也可在“日志与存储”点击立即备份。备份位于 `data/backups/`，自动保留 7 份。
